...
def generate_srt(audio_path: str, enable_vocal_separation: bool = True) -> str:
    """
    ...
    """
    try:
        detailed_result = generate_detailed_transcription(audio_path, enable_vocal_separation)
        return detailed_result["srt_content"]
    except Exception as e:
        # 记录完整的错误栈信息
        error_traceback = traceback.format_exc()
        logger.error(f"SRT生成过程中发生错误: {audio_path}")
        logger.error(f"错误详情: {str(e)}")
        logger.error(f"完整错误栈:\n{error_traceback}")
        raise
